 ...
 * specified.
 */(private val header: UnprotectedHeader, private val encryptedKey: Base64URL) {
    /**
     * Returns the unprotected header for this JWE recipient.
     *
     * @return The unprotected header, `null` if not specified.
     */
    fun getHeader(): UnprotectedHeader? {
        return header
    }
     ...
     *
     * @return The encrypted key, `null` if not specified.
     */
    fun getEncryptedKey(): Base64URL? {
        return encryptedKey
    }
    ...
    fun toJSONObject(): Map<String, Any> {
        ...
        if (getHeader() != null) {
            json["header"] = getHeader()!!.toJSONObject()
        }
        if (getEncryptedKey() != null) {
            json["encrypted_key"] = getEncryptedKey().toString()
        }
        return json
    }
