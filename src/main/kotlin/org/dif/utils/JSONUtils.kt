...
inline fun <reified T> Map<String, Any?>.getTypedArray(key: String): Array<T>? = this[key]?.let {
    if (it !is JSONArray) throw MalformedMessageException("The expected type header '$key' is 'JSONArray'. Got '${it::class.simpleName}'")
    else it.getTyped(key)
}

inline fun <reified T> Map<String, Any?>.getTyped(key: String): T? = this[key]?.let {
    if (it !is T) throw MalformedMessageException("The expected type header '$key' is '${T::class.simpleName}'. Got '${it::class.simpleName}'")
    else it
}
